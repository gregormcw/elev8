...
void ELEV8AudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.setFont(8.0f);
    g.drawFittedText("Designed and built by G. McWilliam", WIN_WIDTH-385, WIN_HEIGHT-20, 380, 20, juce::Justification::right, 1);
}

void ELEV8AudioProcessorEditor::resized()
{
    const int aziSliderWidth  = 220;
    const int aziSliderHeight = 220;
    aziSlider.setBounds(2*getWidth()/3 - aziSliderWidth/2,  getHeight()/2 - aziSliderHeight/2,
                        aziSliderWidth, aziSliderHeight);

    const int eleSliderWidth  = 100;
    const int eleSliderHeight = 220;
